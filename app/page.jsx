...
export default function BusinessApp() {
  ...
  async function loadAppointments(id) { const { data } = await supabase.from("appointments").select("*").eq("business_id", id).order("date", { ascending: true }).order("time", { ascending: true }); setAppointments(data || []); }
  async function loadEmployees(id) { const { data } = await supabase.from("employees").select("*").eq("business_id", id).order("created_at"); setEmployees(data || []); }
  async function loadServices(id) { const { data } = await supabase.from("business_services").select("*").eq("business_id", id).eq("is_active", true).order("created_at"); setServices(data || []); }
  async function loadAvailability(id) { const { data } = await supabase.from("business_availability").select("*").eq("business_id", id).order("day_of_week"); setAvailability(data || []); }
  async function loadBreaks(id) { const { data } = await supabase.from("business_breaks").select("*").eq("business_id", id).order("day_of_week"); setBreaks(data || []); }
  async function loadDaysOff(id) { const { data } = await supabase.from("business_days_off").select("*").eq("business_id", id).order("date", { ascending: false }); setDaysOff(data || []); }
  ...
  const stats = useMemo(() => ({ upcoming: appointments.filter(a => ["pending", "upcoming", "confirmed"].includes(a.status)).length, today: appointments.filter(a => a.date === todayIso()).length, cancelled: appointments.filter(a => a.status === "cancelled").length }), [appointments]);
  ...
  return <div style={S.page}>
    <div style={{ background: "linear-gradient(135deg,#111827,#4f46e5)", color: "white" }}><div style={{ ...S.wrap, display: "flex", justifyContent: "space-between", alignItems: "center", gap: 12 }}>
      ...
      <div style={{ display: "flex", gap: 8, alignItems: "center" }}><button style={{ ...S.btn, background: "rgba(255,255,255,.14)", color: "white" }} onClick={() => navigator.clipboard.writeText(bookingUrl).then(() => notify("הקישור הועתק"))}>🔗 העתק קישור לקוחות</button><button style={{ ...S.btn, background: "rgba(255,255,255,.14)", color: "white" }} onClick={logout}>יציאה</button></div>
    </div></div>
    <div style={{ ...S.wrap, display: "grid", gridTemplateColumns: "220px 1fr", gap: 16 }}>
      <aside style={{ ...S.card, height: "fit-content", position: "sticky", top: 16 }}>
        ...
        {nav.map(([id, label]) => <button key={id} onClick={() => setTab(id)} style={{ ...S.btn, width: "100%", textAlign: "right", marginBottom: 7, background: tab === id ? "#ede9fe" : "transparent", color: tab === id ? "#5b21b6" : "#374151" }}>{label}</button>)}
      </aside>
      <main>{!subActive && tab !== "billing" && <div style={{ ...S.card, borderColor: "#fecaca", background: "#fff7f7", marginBottom: 16 }}><b>המנוי לא פעיל.</b> עמוד הלקוחות של העסק נעול ולא יאפשר קביעת תורים עד הפעלת מנוי. <button style={{ ...S.btn, ...S.primary, marginRight: 12 }} onClick={() => setTab("billing")}>הפעל מנוי</button></div>}
        ...
        {tab === "settings" && <Settings business={business} updateBusiness={updateBusiness} />}
...
function Dashboard({ stats, appointments, bookingUrl }) { return <div style={{ display: "grid", gap: 16 }}><div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 12 }}>{[["תורים פעילים", stats.upcoming], ["היום", stats.today], ["בוטלו", stats.cancelled]].map(([l, v]) => <div key={l} style={S.card}><div style={{ color: "#6b7280", fontWeight: 800 }}>{l}</div><div style={{ fontSize: 34, fontWeight: 900 }}>{v}</div></div>)}</div><div style={S.card}><b>קישור הזמנה ללקוחות</b><div style={{ marginTop: 8, padding: 12, borderRadius: 10, background: "#f3f4f6", direction: "ltr", textAlign: "left", overflow: "auto" }}>{bookingUrl}</div></div><Appointments appointments={appointments.slice(0, 5)} setApptStatus={() => {}} compact /></div>; }
function Appointments({ appointments, setApptStatus, compact }) { return <div style={S.card}><h2 style={{ marginTop: 0 }}>תורים</h2><div style={{ display: "grid", gap: 10 }}>{appointments.length === 0 ? <div style={{ color: "#6b7280" }}>אין תורים עדיין</div> : appointments.map(a => <div key={a.id} style={{ border: "1px solid #e5e7eb", borderRadius: 14, padding: 12, display: "grid", gridTemplateColumns: "1fr auto", gap: 10 }}><div><b>{a.client_name || "לקוח"}</b> · {a.client_phone}<div style={{ color: "#6b7280", fontSize: 13 }}>{a.service} · {a.date} · {safeTime(a.time)} · {a.duration} דק׳</div>{a.notes && <div style={{ color: "#7c3aed", fontSize: 13 }}>הערה: {a.notes}</div>}</div><div style={{ display: "flex", gap: 6, alignItems: "center" }}><span style={{ fontWeight: 900, color: a.status === "cancelled" ? "#dc2626" : a.status === "confirmed" ? "#059669" : "#d97706" }}>{a.status || "pending"}</span>{!compact && <><button style={{ ...S.btn, background: "#dcfce7", color: "#166534" }} onClick={() => setApptStatus(a.id, "confirmed")}>אשר</button><button style={{ ...S.btn, ...S.danger }} onClick={() => setApptStatus(a.id, "cancelled")}>בטל</button></>}</div></div>)}</div></div>; }
function Settings({ business, updateBusiness }) { const [form, setForm] = useState(business); useEffect(() => setForm(business), [business?.id]); return <div style={S.card}><h2>הגדרות עסק</h2><div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}><Field label="שם עסק" value={form.name} onChange={v => setForm({ ...form, name: v })} /><Field label="טלפון" value={form.phone} onChange={v => setForm({ ...form, phone: v })} /><Field label="כתובת" value={form.address} onChange={v => setForm({ ...form, address: v })} /><Field label="קטגוריה" value={form.category} onChange={v => setForm({ ...form, category: v })} /><Field label="קישור לוגו" value={form.logo_url} onChange={v => setForm({ ...form, logo_url: v })} /><Field label="אינסטגרם" value={form.instagram} onChange={v => setForm({ ...form, instagram: v })} /></div><label style={{ display: "block", marginTop: 12 }}><span style={S.label}>אודות העסק</span><textarea style={{ ...S.input, minHeight: 90 }} value={form.about || ""} onChange={e => setForm({ ...form, about: e.target.value })} /></label><button style={{ ...S.btn, ...S.primary, marginTop: 12 }} onClick={() => updateBusiness(form)}>שמור הגדרות</button></div>; }
function Employees({ employees, newEmployee, setNewEmployee, addEmployee, removeEmployee }) { return <div style={S.card}><h2>עובדים</h2><div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr auto", gap: 8, marginBottom: 14 }}><input style={S.input} placeholder="שם" value={newEmployee.name} onChange={e => setNewEmployee({ ...newEmployee, name: e.target.value })} /><input style={S.input} placeholder="תפקיד" value={newEmployee.role} onChange={e => setNewEmployee({ ...newEmployee, role: e.target.value })} /><input style={S.input} placeholder="טלפון" value={newEmployee.phone} onChange={e => setNewEmployee({ ...newEmployee, phone: e.target.value })} /><button style={{ ...S.btn, ...S.primary }} onClick={addEmployee}>הוסף</button></div>{employees.map(e => <div key={e.id} style={{ padding: 12, borderBottom: "1px solid #eee", display: "flex", justifyContent: "space-between" }}><span><b>{e.name}</b> · {e.role} · {e.phone}</span><button style={{ ...S.btn, ...S.danger }} onClick={() => removeEmployee(e.id)}>השבת</button></div>)}</div>; }
function Services({ services, employees, newService, setNewService, addService, removeService }) { return <div style={S.card}><h2>שירותים ומחירים</h2><div style={{ display: "grid", gridTemplateColumns: "70px 1fr 100px 100px 160px auto", gap: 8, marginBottom: 14 }}><input style={S.input} value={newService.emoji} onChange={e => setNewService({ ...newService, emoji: e.target.value })} /><input style={S.input} placeholder="שם שירות" value={newService.name} onChange={e => setNewService({ ...newService, name: e.target.value })} /><input style={S.input} type="number" value={newService.duration} onChange={e => setNewService({ ...newService, duration: Number(e.target.value) })} /><input style={S.input} type="number" value={newService.price} onChange={e => setNewService({ ...newService, price: Number(e.target.value) })} /><select style={S.input} value={newService.employee_id} onChange={e => setNewService({ ...newService, employee_id: e.target.value })}><option value="">כל עובד</option>{employees.map(e => <option key={e.id} value={e.id}>{e.name}</option>)}</select><button style={{ ...S.btn, ...S.primary }} onClick={addService}>הוסף</button></div>{services.map(s => <div key={s.id} style={{ padding: 12, borderBottom: "1px solid #eee", display: "flex", justifyContent: "space-between" }}><span>{s.emoji} <b>{s.name}</b> · {s.duration} דק׳ · ₪{s.price}</span><button style={{ ...S.btn, ...S.danger }} onClick={() => removeService(s.id)}>מחק</button></div>)}</div>; }
function Availability({ availability, updateAvailability, breaks, addBreak, newBreak, setNewBreak, daysOff, addDayOff, newDayOff, setNewDayOff }) { const rows = DAYS.map((d, i) => availability.find(r => r.day_of_week === i) || { day_of_week: i, is_open: i !== 6, start_time: "09:00", end_time: i === 5 ? "14:00" : "18:00", slot_minutes: 30 }); return <div style={{ display: "grid", gap: 16 }}><div style={S.card}><h2>זמינות שבועית</h2>{rows.map(r => <div key={r.day_of_week} style={{ display: "grid", gridTemplateColumns: "50px 100px 1fr 1fr 1fr auto", gap: 8, alignItems: "center", marginBottom: 8 }}><b>{DAYS[r.day_of_week]}</b><label><input type="checkbox" checked={!!r.is_open} onChange={e => updateAvailability({ ...r, is_open: e.target.checked })} /> פתוח</label><select style={S.input} value={safeTime(r.start_time)} onChange={e => updateAvailability({ ...r, start_time: e.target.value })}>{TIMES.map(t => <option key={t}>{t}</option>)}</select><select style={S.input} value={safeTime(r.end_time)} onChange={e => updateAvailability({ ...r, end_time: e.target.value })}>{TIMES.map(t => <option key={t}>{t}</option>)}</select><select style={S.input} value={r.slot_minutes || 30} onChange={e => updateAvailability({ ...r, slot_minutes: Number(e.target.value) })}><option value="15">15 דק׳</option><option value="30">30 דק׳</option><option value="60">60 דק׳</option></select><button style={{ ...S.btn, ...S.ghost }} onClick={() => updateAvailability(r)}>שמור</button></div>)}</div><div style={S.card}><h2>הפסקות קבועות</h2><div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr 1fr auto", gap: 8 }}><select style={S.input} value={newBreak.day_of_week} onChange={e => setNewBreak({ ...newBreak, day_of_week: Number(e.target.value) })}>{DAYS.map((d, i) => <option key={d} value={i}>{d}</option>)}</select><input style={S.input} value={newBreak.start_time} onChange={e => setNewBreak({ ...newBreak, start_time: e.target.value })} /><input style={S.input} value={newBreak.end_time} onChange={e => setNewBreak({ ...newBreak, end_time: e.target.value })} /><input style={S.input} value={newBreak.title} onChange={e => setNewBreak({ ...newBreak, title: e.target.value })} /><button style={{ ...S.btn, ...S.primary }} onClick={addBreak}>הוסף</button></div>{breaks.map(b => <div key={b.id} style={{ padding: 8 }}>{DAYS[b.day_of_week]} · {safeTime(b.start_time)}-{safeTime(b.end_time)} · {b.title}</div>)}</div><div style={S.card}><h2>חופשות וימים סגורים</h2><div style={{ display: "grid", gridTemplateColumns: "1fr 2fr auto", gap: 8 }}><input style={S.input} type="date" value={newDayOff.date} onChange={e => setNewDayOff({ ...newDayOff, date: e.target.value })} /><input style={S.input} placeholder="סיבה" value={newDayOff.reason} onChange={e => setNewDayOff({ ...newDayOff, reason: e.target.value })} /><button style={{ ...S.btn, ...S.primary }} onClick={addDayOff}>הוסף</button></div>{daysOff.map(d => <div key={d.id} style={{ padding: 8 }}>{d.date} · {d.reason}</div>)}</div></div>; }
function Billing({ subscription, startCheckout, loading }) { const active = isActiveSubscription(subscription); return <div style={S.card}><h2>מנוי ותשלום</h2><div style={{ padding: 12, borderRadius: 12, background: active ? "#dcfce7" : "#fee2e2", color: active ? "#166534" : "#991b1b", marginBottom: 16, fontWeight: 900 }}>{active ? `מנוי פעיל: ${subscription.plan_id}` : "אין מנוי פעיל — עמוד ההזמנות נעול"}</div><div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 12 }}>{PLANS.map(p => <div key={p.id} style={{ border: "1px solid #e5e7eb", borderRadius: 16, padding: 16 }}><div style={{ fontWeight: 900, fontSize: 20 }}>{p.name}</div><div style={{ color: "#7c3aed", fontSize: 30, fontWeight: 900 }}>₪{p.price}</div><div style={{ color: "#6b7280" }}>לחודש</div><ul>{p.features.map(f => <li key={f}>{f}</li>)}</ul><button style={{ ...S.btn, ...S.primary, width: "100%" }} disabled={loading} onClick={() => startCheckout(p.id)}>בחר חבילה</button></div>)}</div></div>; }
